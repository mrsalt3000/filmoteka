...
После проверок и отчёта — коммит и пуша.

- Стейжинг: только файлы, относящиеся к задаче. Не включать `.qwen/settings.json`, `.qwen/settings.json.orig` и другие локальные артефакты.
- Формат: `type: краткое описание` (subject до 72 символов, без точки в конце) + `TASK-ID: описание` в теле.
...
- [ ] пуш выполнен сразу после коммита;
