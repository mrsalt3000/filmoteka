...
Поддерживаемые форматы: MP4 (родной для браузеров), MKV (зависит от
браузера). Для несовместимых форматов используйте функцию
"Transcode audio" в admin UI.
...
- **MKV** — для браузеров, не поддерживающих MKV, используйте
  функцию "Transcode audio" в admin UI (создаёт .tr.mp4 копию).
  Без транскодирования MKV передаётся как есть — браузер может
  не поддерживать воспроизведение.
- **HD (1080p)** работает без проблем даже по WiFi.
- **4K HDR** с высоким битрейтом может требовать проводного
  подключения сервера к роутеру.
...
- **Поиск** — использует PostgreSQL full-text search с русским словарём.
  Поиск по названиям на латинице может работать хуже из-за стемминга.
